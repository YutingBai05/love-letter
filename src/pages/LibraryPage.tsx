...
function LetterCard({ letter }: { letter: Letter }) {
  ...
  const getPaperClass = (tpl: string) => {
    switch (tpl) {
      ...
      case 'grid': return 'bg-[repeating-linear-gradient(rgba(180,160,140,0.15)_0,rgba(180,160,140,0.15)_1px,transparent_1px,transparent_20px),repeating-linear-gradient(90deg,rgba(180,160,140,0.15)_0,rgba(180,160,140,0.15)_1px,transparent_1px,transparent_20px)]'
  ...
  return (
    <div className="rounded-xl p-5 bg-white/70 border border-warm-beige">
      <div className="flex items-center gap-2 mb-2">
        ...
        <button onClick={(e) => { e.stopPropagation(); exportQA(answer) }}
...
export function LibraryPage() {
  ...
  const [allQA, setAllQA] = useState<QAAnswer[]>([])

  useEffect(() => {
    getFolders(user?.role).then(setFolders)
    getPostcards().then(setAllPostcards)
    getLetters().then(setAllLetters)
    getAnsweredQAHistory().then(setAllQA)
  }, [user])

  const filteredPostcards = useMemo(() => {
    const list = folderFilter === 'all' ? allPostcards : allPostcards.filter((p) => p.folderId === folderFilter)
    return list
  }, [folderFilter, allPostcards])

  const filteredLetters = useMemo(() => {
    const list = folderFilter === 'all' ? allLetters : allLetters.filter((l) => l.folderId === folderFilter)
    return list
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 bg-warm-beige/50 rounded-lg p-1">
        {([
          ['postcards', '明信片', allPostcards.length],
          ['letters', '信件', allLetters.length],
          ['qa', '问答', allQA.length],
        ] as const).map(([v, label, count]) => (
          <button key={v} onClick={() => setTab(v)}
            className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors ${tab === v ? 'bg-white text-rose shadow-sm' : 'text-warm-gray hover:text-ink-brown'}`}>
            {label} ({count})
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {tab !== 'qa' && (
          <button onClick={() => {
            if (tab === 'postcards') filteredPostcards.forEach((p) => markAsRead(p.id))
            else filteredLetters.forEach((l) => markLetterAsRead(l.id))
          }} className="text-xs text-rose hover:text-rose/80 transition-colors">全部标为已读</button>
        )}
      </div>

      <div className="space-y-3">
        {tab === 'postcards' && (filteredPostcards.length === 0 ? (
          <div className="text-center py-12 text-warm-gray text-sm">还没有明信片，去写一张吧</div>
        ) : filteredPostcards.map((p) => (
          <div key={p.id}>
            <div onClick={() => markAsRead(p.id)}><PostcardCard postcard={p} /></div>
            <div className="flex items-center gap-3 mt-1 ml-1">
              {!p.read && <span className="px-1.5 py-0.5 rounded bg-red-100 text-red-500 text-[10px]">未读</span>}
              <button onClick={() => exportPostcard(p)} className="flex items-center gap-1 text-xs text-warm-gray hover:text-rose transition-colors">
                <Download className="w-3 h-3" />导出
              </button>
            </div>
          </div>
        ...
        {tab === 'letters' && (filteredLetters.length === 0 ? (
          <div className="text-center py-12 text-warm-gray text-sm">还没有信件，去写一封吧</div>
        ) : filteredLetters.map((l) => (
          <div key={l.id}>
            <div onClick={() => markLetterAsRead(l.id)}><LetterCard letter={l} /></div>
            {!l.read && <span className="inline-block ml-2 px-1.5 py-0.5 rounded bg-red-100 text-red-500 text-[10px]">未读</span>}
          </div>
        )))}

        {tab === 'qa' && (allQA.length === 0 ? (
          <div className="text-center py-12 text-warm-gray text-sm">还没有完成的问答，去答一题吧</div>
        ) : allQA.map((a) => <QACard key={a.id} answer={a} />))}
      </div>
    </div>
  )
}
